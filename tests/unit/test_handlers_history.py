...
"""Tests unitarios de handlers ``history.*``."""
...
async def test_undo_empty_stack_returns_applied_false() -> None:
    result = await history_handlers.undo({})
    assert result == {"applied": False, "reason": "empty"}


async def test_undo_after_extrude_restores_wall_mesh() -> None:
    ...
    vec = [0.25 * nx, 0.25 * ny, 0.25 * nz]

    await geom_handlers.extrude_face({"topo_id": topo_id, "vector": vec})

    undone = await history_handlers.undo({})
    assert undone["applied"] is True
    assert undone["guid"] == wall["guid"]
    assert Mesh.from_dict(undone["mesh"]).vertices == Mesh.from_dict(before).vertices

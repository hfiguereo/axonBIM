...
La **malla 3D** y las **proyecciones 2D** vectoriales (`draw.ortho_snapshot`) comparten el **mismo pipeline geométrico analítico** en Python: no hay un segundo motor de sólidos paralelo, de modo que líneas en planta/alzado corresponden al mismo modelo que el visor. Detalle para desarrollo: [`docs/architecture/geometry-analytical.md`](architecture/geometry-analytical.md).
...
| **Rendimiento / GPU** | Por defecto el motor usa **OpenGL (GL Compatibility)** para arranque estable. Si en *Project Settings → Rendering* activas **Forward+ (Vulkan)** y aparece **ventana negra**, cuelgue o **SIGABRT**, vuelve a **GL Compatibility**. En Linux, `./start` respeta **`AXONBIM_GPU_PROFILE`** (`auto`, `integrated`, `dedicated`); matriz, checklist y Flatpak+NVIDIA: [**README**](../README.md) y [**ADR-0005**](architecture/decisions/0005-renderizado-godot-gl-default-y-perfiles-gpu.md). La pestaña **Preferencias** de la cinta permite guardar el motor en `project.godot` (reinicio) y escribir **`.env.axonbim`** con el perfil GPU. |
| **Selección** | Elige una entidad en el modelo para inspeccionarla o combinarla con otras acciones. |
| **Crear muro** | **Nivel de trabajo:** en **Propiedades**, **Nivel activo (IFC)** elige el ``IfcBuildingStorey`` donde caen los muros nuevos; **Añadir nivel** crea otro forjado con la cota indicada (metros en Z). El trazo en 3D y en **vista 2D** usa la **cota Z** de ese nivel. En **vista 3D** el muro se define en el **plano horizontal** (eje **Z** arriba; huella en **X/Y**). En **vista 2D** (snapshot vectorial u ortográfica) el trazo usa **esa misma huella en X/Y** leyendo la geometría de la vista, **sin** tomar la **cámara 3D** como referencia del dibujo. El **primer** segmento pide clic **P1** y clic **P2**; cada muro válido sigue desde el **final (P2) del anterior**, de modo que el siguiente trazo suele ser **solo un clic (P2)**. **Alt + clic** fuerza **nuevo P1** (también reinicia el anclaje de **cierre de habitación**). Al volver con el último **P2** cerca del **primer vértice** del trazo, el editor **acopla** ese punto y el backend cierra la esquina como en el resto de la cadena. **Esc** o el mismo botón cierran la herramienta. Inferencia a ejes **X/Y** desde P1 y a **X=0 / Y=0**. **Altura y espesor** en **Propiedades** (tipología/familia). La pista bajo el visor muestra **largo en planta**, altura/espesor, etc., al mover el ratón durante el trazo. |
| **Datums de nivel (3D)** | En **vista 3D**, cada ``IfcBuildingStorey`` muestra un **rectángulo de referencia** en el plano XY a su cota Z, una **etiqueta** y un **grip** (esfera) seleccionable con clic cuando no hay otra herramienta activa. El **Navegador de proyecto** incluye **IFC → Niveles**. En **Propiedades**, **Nivel seleccionado (datum 3D)** permite editar nombre y cota (**Aplicar al IFC** llama a ``project.update_storey``); **Nivel activo** fija el forjado de trabajo para muros nuevos (``project.set_active_storey``). |
| **Push/Pull** | Modelado directo: tras **fijar una cara** (modo edición), arrastrar o introducir **distancia numérica** en Propiedades para extruir según el vector indicado. Con el ratón sobre el modelo, el resaltado de hover agrupa la **cara lógica** (todos los triángulos que comparten el mismo `topo_id`), no solo el triángulo bajo el cursor. Con `AXONBIM_LOG_LEVEL=DEBUG`, Godot puede registrar en consola `debug_mesh_stats` devuelto por el backend tras aplicar la extrusión (solo diagnóstico). |
...
| **Generar vistas 2D…** | Exporta **Planta + N/S/E/O** vía `draw.ortho_snapshot` (proyección analítica) a PNG técnico raster en Godot; si el snapshot falla, cae a la ruta legacy (captura ortográfica del viewport) sin bloquear la UI. |
...
| **Vistas 2D en Project Browser** | En `Navegador de proyecto` aparece `Vistas 2D` con **Planta** y elevaciones **Norte / Sur / Este / Oeste** respecto al **norte de proyecto (+Y)**. Abrir una vista desde el árbol crea una **pestaña** (solo se muestran pestañas si hay más de una vista abierta; la base es **Modelado 3D**). Puedes crear vistas adicionales con `+ Vista 2D` y cerrar la pestaña activa con `Eliminar vista`. Cada vista mantiene estado (`loading/ready/error/fallback`) y escala aproximada por vista durante la sesión activa; si el snapshot vectorial falla, vuelve automáticamente al preset ortográfico legacy. En vista 2D vectorial: **rueda = zoom**, **MMB arrastre = pan**. |
| **View Range (planta 2D)** | La planta en modo vectorial usa rango de vista configurable (`cut/top/bottom/depth`). Atajos iniciales: **PgUp/PgDn** ajustan el plano de corte (`cut_plane`) en pasos de 0,10 m. El HUD muestra zoom + valores activos del rango. |
...
| [`docs/architecture/geometry-analytical.md`](architecture/geometry-analytical.md) | Qué significa el pipeline analítico único (3D + snapshot 2D). |
